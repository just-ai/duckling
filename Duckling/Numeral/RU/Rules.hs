...
ruleDecimalNumeralComma :: Rule
ruleDecimalNumeralComma = Rule
  { name = "decimal number (comma)"
  , pattern =
    [ regex "(\\d*,\\d+)"
    ]
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):_) -> parseDecimal True match2
        where match2 = (Text.replace "," "." match)
      _ -> Nothing
  }
...
rules =
  [ ruleDecimalNumeral
  , ruleDecimalNumeralComma
  , ruleDecimalWithThousandsSeparator
  , ruleInteger
  , ruleInteger2
  , ruleInteger3
  , ruleInteger4
  , ruleInteger4Genitive -- todo add other genitives
  , ruleInteger5
  , ruleInteger6
  , ruleInteger7
  , ruleInteger8
